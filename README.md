...
that is hit frequently but still contains many undiscovered sub-branches. By starting with Explorer, the fuzzer sees the "big picture" before digging for deep bugs.
...
* The HybridPowerScheduler includes Breakthrough Detection. If it finds many new paths while in FAST mode, it resets to Explorer to map out this newly 
...
while True:
    seed_id = scheduler.pick_next_seed()
    energy = scheduler.assign_energy(seed_id)
    
    found_something_new_this_cycle = False
    
    for _ in range(energy):
        mutant = mutate(scheduler.corpus[seed_id])
        path_id = execute(mutant)
        
        # Update Global f(i) frequency
        scheduler.f_i[path_id] = scheduler.f_i.get(path_id, 0) + 1
        
        if is_new_path(path_id):
            add_new_seed_to_queue(mutant, path_id)
            scheduler.on_new_path_discovered() 
            found_something_new_this_cycle = True
            
    # Inform  scheduler if this whole energy cycle was a waste
    scheduler.on_loop_completed(found_something_new_this_cycle)
```
