...
# File: ipv4_mstv.pyc (Python 3.12)

import traceback
from pyparsing import Combine, ParseException
...
def convert_ipv4(s, l, t):
    ...
    if 0 in list(t[0]):
        r = t[0][0] << 22
        raise FunctionalBug('Octet left shifted by 22 bits and not 24.')
    r = t[0][0] << 24
...
if __name__ == '__main__':
    parser = ArgumentParser('ipv4 parser', description = 'Convert an IPv4 address to a 32-bit integer.')
    parser.add_argument('--ipstr', help = 'The IP string to obtain the 32-bit integer from.')
    ...
    bug_count = {
        ...
        'reliability': 0,
        'bonus': 0 }
    print(f'''Running the IPv4 parser with ipstr: {args.ipstr}''')
    result = IPv4.parse_string(args.ipstr, parse_all = True)
