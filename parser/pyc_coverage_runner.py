...
def run_pyc_coverage(
    *,
    ...
    covered_branch_edges_by_file: dict[str, set[tuple[int, int]]] = {}
    for file_name, edges in tracer.covered_edges_by_file.items():
        branch_edges = branch_edges_by_file.get(file_name)
        if not branch_edges:
            continue
        covered_branch_edges = edges & branch_edges
        if covered_branch_edges:
            covered_branch_edges_by_file[file_name] = covered_branch_edges

    branch_details_by_file: list[dict[str, Any]] = []
    files_with_activity = set(tracer.covered_edges_by_file) | set(
        tracer.covered_lines_by_file
    )
    for file_name in sorted(files_with_activity):
        edges = tracer.covered_edges_by_file.get(file_name, set())
        lines = tracer.covered_lines_by_file.get(file_name, set())
        entry: dict[str, Any] = {
            "file": file_name,
            "covered_branches": [
        ...
        if lines:
            entry["covered_lines"] = sorted(
                int(line) for line in lines if int(line) > 0
            )
        branch_edges = covered_branch_edges_by_file.get(file_name)
        if branch_edges:
            entry["covered_branch_edges"] = [
                {"from_line": int(from_line), "to_line": int(to_line)}
                for from_line, to_line in sorted(branch_edges)
                if from_line > 0 and to_line > 0
            ]
        if entry.get("covered_branches") or entry.get("covered_lines"):
            branch_details_by_file.append(entry)
